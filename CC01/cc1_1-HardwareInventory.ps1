...
foreach ($pc in $pcs) {
    ...
    $computerGPU = Get-WmiObject Win32_VideoController | Select description,driverversion,CurrentHorizontalResolution,CurrentVerticalResolution
    ...
    $tempval | add-member -membertype noteproperty -name DiskSize -value $computerDisks[0].Size
    $tempval | add-member -membertype noteproperty -name DiskFreeSpace -value $computerDisks[0].FreeSpace
